...
export default function Page() {
  ...
  return (
    <AppShell title="Usuários" description="Controle de acesso e perfis">
      <div className="page-actions"><button className="btn" onClick={() => setOpen(true)}>+ Novo usuário</button></div>
      <Modal open={open} title="Novo usuário" onClose={() => setOpen(false)}>
        <form className="panel form" onSubmit={add}>
          ...
          <div className="full actions"><button type="button" className="btn secondary" onClick={() => setOpen(false)}>Cancelar</button><button className="btn">Criar usuário</button></div>
        </form>
      </Modal>
      <div className="panel"><div className="table-wrap"><table className="table"><thead><tr><th>Nome</th><th>E-mail</th><th>Perfil</th><th>Status</th></tr></thead><tbody>{items.map((x) => <tr key={x.id}><td>{x.name}</td><td>{x.email}</td><td>{x.role}</td><td>{x.active ? "Ativo" : "Inativo"}</td></tr>)}</tbody></table></div></div>
    </AppShell>
  );
}
